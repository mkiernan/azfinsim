...
from . import azlog, xmlutils, utils, getargs
...
def create_trade_range(start_trade, r):
    batchsent=0
...
def execute(args):
    ...
    if (args.cache_type == "redis" or args.cache_type == "hazelcast"):
        log.info('connecting to %s', args.cache_type)
...
#   thread_pool.map(create_trades, range(start_trade,stop_trade))
    thread_pool.map(lambda x: create_trade_range(x, r), range(start_trade, stop_trade, batchsize))
...
if __name__ == "__main__":
    #-- grab cli args
    args = getargs.getargs("generator")
    execute(args)
